...
- `cbps` Puerto Rico CSV header (`cbp14pr.csv`):
  `fipstate,naics,...,ap,...,est,...,censtate`
- `cbps` municipality CSV header (`cbp14pr_mun.csv`):
  `fipstate,fipscty,naics,...,ap,est,...,cencty`
...
| `created_at`, `updated_at` | generated | derived | Generated at import time. |
...
not include an unambiguous `total_indus` alias.
