...
| FR-018 | The product shall keep reviewed decision-signal upgrades as descriptive evidence artifacts that cite official Puerto Rico authorities and explicit interpretation limits. | `data/profile-reach/aguada-restaurant-permit-path-review.json` and `data/profile-reach/aguada-restaurant-utility-service-review.json` record reviewed upgrades for the Aguada restaurant regulatory and infrastructure signals, while `npm run test:regulatory-signal-review` and `npm run test:infrastructure-signal-review` verify their authority stacks, fixed-selection linkage, and descriptive-only limits. |
...
| FR-018 | Reviewed decision-signal evidence upgrades | `docs/product-scope.md`, `docs/data-intake.md`, `data/profile-reach/aguada-restaurant-permit-path-review.json`, `data/profile-reach/aguada-restaurant-utility-service-review.json`, `npm run test:regulatory-signal-review`, `npm run test:infrastructure-signal-review` |
