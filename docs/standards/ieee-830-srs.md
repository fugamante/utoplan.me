...
| FR-018 | The product shall keep reviewed decision-signal upgrades as descriptive evidence artifacts that cite official Puerto Rico authorities and explicit interpretation limits. | `data/profile-reach/aguada-restaurant-permit-path-review.json` records the first reviewed upgrade for the Aguada restaurant permit-path signal, and `npm run test:regulatory-signal-review` verifies its authority stack, fixed-selection linkage, and descriptive-only limits. |
...
| FR-018 | Reviewed decision-signal evidence upgrades | `docs/product-scope.md`, `docs/data-intake.md`, `data/profile-reach/aguada-restaurant-permit-path-review.json`, `npm run test:regulatory-signal-review` |
