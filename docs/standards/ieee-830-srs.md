...
| DR-002C | Active mapped-table candidates that derive preserved coordinates through an external geocoder shall pin a reproducible geocoding policy, checked-in cache artifact path, and a reviewed quarantine artifact for non-exact rows before import. | `npm run test:data-sources` enforces the `geocodingPolicy` contract for `unis`, while `docs/unis-geocoding-policy.md` records the exact-match-only workflow. |
...
- The approved `cbps.cnaic_name` auxiliary join now depends on the checked-in
  ...
  command.
- The `unis` replacement candidate now depends on keeping non-exact
  institution-name rows quarantined beyond the strict 11-of-57 exact-match
  baseline recorded in `data/unis/ipeds-geocode-audit.json` unless the source
