...
| Reviewed signal-upgrade artifacts | `data/profile-reach/aguada-restaurant-permit-path-review.json`, `data/profile-reach/aguada-restaurant-utility-service-review.json` | Record bounded reviewed evidence upgrades for individual decision signals without expanding API/UI scope or turning process-path or continuity-baseline evidence into recommendations. |
