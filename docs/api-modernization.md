...
- `dtoapi/modern/test/anonymous_endpoint_contract_test.js` pins endpoint-level reserved anonymous route contracts, including method allowlists, allowed/denied CORS behavior, same-origin Referer fallback, no cookie issuance, and exact `501`/`403`/`405` response envelopes while runtime behavior remains disabled.
...
Add rate-limit response contract tests and anonymous endpoint handler composition before implementing anonymous profile routes under `dtoapi/modern/src/`. Test migration to TypeScript is optional and lower priority than product behavior or CI hardening.
