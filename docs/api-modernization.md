...
- Anonymous rate-limit and profile body-validation scaffolding is wired into the gated runtime. Mounted runtime handlers now accept async limiter decisions, and shared mode uses Postgres-backed `anonymous_rate_limit_buckets` storage when enabled. The process-local fixed-window helper remains for unit tests and reserved-route scaffolding. Profile validation enforces the 2048-byte body cap before parsing, allowed fields only, positive integer municipality ids, category id pattern checks, and 160-character `businessIdea` limit.
...
- Anonymous runtime activation is wired fail-closed. `UTOPLAN_ANONYMOUS_RUNTIME=1` can only reach handler success behavior after anonymous schema readiness is confirmed and `UTOPLAN_ANONYMOUS_RATE_LIMIT_MODE` is `shared` or `edge`; edge mode requires `UTOPLAN_ANONYMOUS_EDGE_RATE_LIMIT=1`, and shared mode requires trusted proxy client identity plus `UTOPLAN_ANONYMOUS_SHARED_RATE_LIMIT=1`.
- Disposable anonymous runtime validation is available through `npm run docker:test:anonymous-runtime`. It builds a throwaway Postgres image with the base contract data, anonymous storage tables, and `anonymous_rate_limit_buckets`, then runs the opt-in anonymous release smoke through the app proxy on local ports `18084` app, `13001` API, and `15433` Postgres.
...
Next, promote the disposable anonymous-runtime smoke into the regular release checklist only after a final audit of proxy/header trust behavior, CORS/CSRF expectations, and migration rollback notes. Test migration to TypeScript is optional and lower priority than product behavior or CI hardening.
