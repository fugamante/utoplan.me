...
Next, apply the anonymous storage and shared-limiter migration artifacts in a disposable environment, enable shared anonymous runtime config, and run the opt-in anonymous release smoke against that candidate. Test migration to TypeScript is optional and lower priority than product behavior or CI hardening.
