...
var artifactPath = path.join(__dirname, '..', 'data', 'profile-reach', 'aguada-restaurant-corridor-logistics-review.json');
...
var expectedAuthorityIds = [
  'prhta-pr2-aguadilla-mayaguez-corridor-study-2024',
  'iepr-air-maritime-cargo-passengers-2024-q2'
];

assert.strictEqual(artifact.schemaVersion, 1, 'artifact schemaVersion must be 1');
assert.strictEqual(artifact.artifactVersion, 'aguada-restaurant-corridor-logistics-review-v1', 'artifactVersion mismatch');
...
assert.strictEqual(artifact.signalId, 'cold-chain-corridor-gap', 'artifact signalId mismatch');
assert(signal, 'artifact signalId must exist in the decision-signal registry');
assert.strictEqual(signal.sourceType, 'registered-source', 'logistics signal must be registered-source');
assert.strictEqual(signal.sourceId, 'prhta-pr2-aguadilla-mayaguez-corridor-study-2024', 'logistics signal sourceId mismatch');
...
assert.deepStrictEqual(
  ...
  'authority sourceIds must follow the reviewed logistics stack'
);
...
assert(signal.artifactPaths.indexOf('data/profile-reach/aguada-restaurant-corridor-logistics-review.json') !== -1, 'signal must link to the reviewed logistics artifact');
...
assert.strictEqual(artifact.scenarioCoverage[0].coverage, 'descriptive-corridor-logistics-baseline', 'scenarioCoverage coverage mismatch');
