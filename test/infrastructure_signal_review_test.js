...
var artifactPath = path.join(__dirname, '..', 'data', 'profile-reach', 'aguada-restaurant-utility-service-review.json');
...
var expectedAuthorityIds = [
  'pr-energy-bureau-2026-electricity-rate-order',
  'luma-outage-reporting-faq',
  'prasa-aguada-water-service-interruption-2023'
];

assert.strictEqual(artifact.schemaVersion, 1, 'artifact schemaVersion must be 1');
assert.strictEqual(artifact.artifactVersion, 'aguada-restaurant-utility-service-review-v1', 'artifactVersion mismatch');
...
assert.strictEqual(artifact.signalId, 'aguada-basic-utility-service-baseline', 'artifact signalId mismatch');
assert(signal, 'artifact signalId must exist in the decision-signal registry');
assert.strictEqual(signal.sourceType, 'registered-source', 'infrastructure signal must be registered-source');
...
assert.deepStrictEqual(
  ...
  'authority sourceIds must follow the reviewed infrastructure stack'
);
...
assert(signal.artifactPaths.indexOf('data/profile-reach/aguada-restaurant-utility-service-review.json') !== -1, 'signal must link to the reviewed utility artifact');

assert(Array.isArray(artifact.scenarioCoverage), 'scenarioCoverage must be an array');
assert.strictEqual(artifact.scenarioCoverage.length, 1, 'scenarioCoverage must cover small-local only');
assert.strictEqual(artifact.scenarioCoverage[0].scenarioId, 'small-local', 'scenarioCoverage scenarioId mismatch');
assert.strictEqual(artifact.scenarioCoverage[0].reachId, 'local-catchment', 'scenarioCoverage reachId mismatch');
assert.strictEqual(artifact.scenarioCoverage[0].coverage, 'descriptive-utility-baseline', 'scenarioCoverage coverage mismatch');
