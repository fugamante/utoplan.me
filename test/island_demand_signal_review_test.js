...
var artifactPath = path.join(__dirname, '..', 'data', 'profile-reach', 'aguada-restaurant-island-demand-review.json');
...
var expectedAuthorityIds = [
  'pr-tourism-visitors-expenditures-contribution-2024',
  'pr-tourism-passenger-movement-cy-2024'
];

assert.strictEqual(artifact.schemaVersion, 1, 'artifact schemaVersion must be 1');
assert.strictEqual(artifact.artifactVersion, 'aguada-restaurant-island-demand-review-v1', 'artifactVersion mismatch');
...
assert.strictEqual(artifact.signalId, 'island-wide-destination-demand-gap', 'artifact signalId mismatch');
assert(signal, 'artifact signalId must exist in the decision-signal registry');
assert.strictEqual(signal.sourceType, 'registered-source', 'island demand signal must be registered-source');
assert.strictEqual(signal.sourceId, 'pr-tourism-visitors-expenditures-contribution-2024', 'island demand signal sourceId mismatch');
...
assert.deepStrictEqual(
  ...
  'authority sourceIds must follow the reviewed island-demand stack'
);
...
assert(signal.artifactPaths.indexOf('data/profile-reach/aguada-restaurant-island-demand-review.json') !== -1, 'signal must link to the reviewed island-demand artifact');
...
assert.strictEqual(artifact.scenarioCoverage[0].reachId, 'island-wide', 'scenarioCoverage reachId mismatch');
assert.strictEqual(artifact.scenarioCoverage[0].coverage, 'descriptive-island-demand-baseline', 'scenarioCoverage coverage mismatch');
