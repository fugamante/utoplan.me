...
var artifactPath = path.join(__dirname, '..', 'data', 'profile-reach', 'aguada-restaurant-workforce-pipeline-review.json');
...
var expectedAuthorityIds = [
  ...
  'pr-lmi-occupation-projections-2024-2034'
];

assert.strictEqual(artifact.schemaVersion, 1, 'artifact schemaVersion must be 1');
assert.strictEqual(artifact.artifactVersion, 'aguada-restaurant-workforce-pipeline-review-v1', 'artifactVersion mismatch');
...
assert.strictEqual(artifact.signalId, 'specialized-management-pipeline-gap', 'artifact signalId mismatch');
assert(signal, 'artifact signalId must exist in the decision-signal registry');
assert.strictEqual(signal.sourceType, 'registered-source', 'workforce signal must be registered-source');
...
assert.deepStrictEqual(
  ...
  'authority sourceIds must follow the reviewed workforce stack'
);
...
assert(signal.artifactPaths.indexOf('data/profile-reach/aguada-restaurant-workforce-pipeline-review.json') !== -1, 'signal must link to the reviewed workforce artifact');
...
assert.strictEqual(artifact.scenarioCoverage[0].coverage, 'descriptive-workforce-baseline', 'scenarioCoverage coverage mismatch');
