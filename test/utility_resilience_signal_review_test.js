...
var artifactPath = path.join(__dirname, '..', 'data', 'profile-reach', 'aguada-restaurant-utility-resilience-review.json');
...
var expectedAuthorityIds = [
  'pr-energy-bureau-jan-2026-reliability-metrics-report',
  'pr-energy-bureau-reliability-metrics-order-2025',
  'pr-energy-bureau-essa-reliability-order-2026',
  'prasa-hurricane-season-guidance',
  'prasa-mission-operations-profile'
];

assert.strictEqual(artifact.schemaVersion, 1, 'artifact schemaVersion must be 1');
assert.strictEqual(artifact.artifactVersion, 'aguada-restaurant-utility-resilience-review-v1', 'artifactVersion mismatch');
...
assert.strictEqual(artifact.signalId, 'utility-resilience-gap', 'artifact signalId mismatch');
assert(signal, 'artifact signalId must exist in the decision-signal registry');
assert.strictEqual(signal.sourceType, 'registered-source', 'utility resilience signal must be registered-source');
assert.strictEqual(signal.sourceId, 'pr-energy-bureau-jan-2026-reliability-metrics-report', 'utility resilience signal sourceId mismatch');
...
assert.deepStrictEqual(
  ...
  'authority sourceIds must follow the reviewed utility resilience stack'
);
...
artifact.authorities.forEach(function(authority) {
  ...
  assert.strictEqual(source.targetTables[0], 'planning-profile-infrastructure', authority.sourceId + ' must target planning-profile-infrastructure');
...
assert(signal.artifactPaths.indexOf('data/profile-reach/aguada-restaurant-utility-resilience-review.json') !== -1, 'signal must link to the reviewed utility resilience artifact');

assert(Array.isArray(artifact.scenarioCoverage), 'scenarioCoverage must be an array');
assert.strictEqual(artifact.scenarioCoverage.length, 2, 'scenarioCoverage must cover medium-regional and large-strategic only');
artifact.scenarioCoverage.forEach(function(entry) {
  assert(entry.scenarioId === 'medium-regional' || entry.scenarioId === 'large-strategic', 'unexpected scenarioCoverage scenarioId: ' + entry.scenarioId);
  assert(entry.reachId === 'regional-corridor' || entry.reachId === 'island-wide', 'unexpected scenarioCoverage reachId: ' + entry.reachId);
  assert.strictEqual(entry.coverage, 'descriptive-utility-resilience-baseline', 'scenarioCoverage coverage mismatch for ' + entry.scenarioId);
