...
var artifactPath = path.join(__dirname, '..', 'data', 'profile-reach', 'aguada-restaurant-external-logistics-review.json');
...
var expectedAuthorityIds = [
  'iepr-air-maritime-cargo-passengers-2024-q2'
];

assert.strictEqual(artifact.schemaVersion, 1, 'artifact schemaVersion must be 1');
assert.strictEqual(artifact.artifactVersion, 'aguada-restaurant-external-logistics-review-v1', 'artifactVersion mismatch');
...
assert.strictEqual(artifact.reviewedAt, '2026-08-13', 'artifact reviewedAt mismatch');
assert.strictEqual(artifact.signalId, 'external-supply-chain-gap', 'artifact signalId mismatch');
assert(signal, 'artifact signalId must exist in the decision-signal registry');
assert.strictEqual(signal.sourceType, 'registered-source', 'external logistics signal must be registered-source');
assert.strictEqual(signal.sourceId, 'iepr-air-maritime-cargo-passengers-2024-q2', 'external logistics signal sourceId mismatch');
...
assert.deepStrictEqual(
  ...
  'authority sourceIds must follow the reviewed external-logistics stack'
);
...
artifact.authorities.forEach(function(authority) {
  ...
  assert.strictEqual(source.targetTables[0], 'planning-profile-logistics', authority.sourceId + ' must target planning-profile-logistics');
...
assert(!signal.supportingSourceIds, 'external logistics signal should not require supportingSourceIds for the current one-source review');
assert(signal.artifactPaths.indexOf('data/profile-reach/aguada-restaurant-external-logistics-review.json') !== -1, 'signal must link to the reviewed external logistics artifact');
...
assert.strictEqual(artifact.scenarioCoverage[0].reachId, 'external-connection', 'scenarioCoverage reachId mismatch');
assert.strictEqual(artifact.scenarioCoverage[0].coverage, 'descriptive-external-logistics-baseline', 'scenarioCoverage coverage mismatch');
