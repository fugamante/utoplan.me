...
var artifactPath = path.join(__dirname, '..', 'data', 'profile-reach', 'aguada-restaurant-support-network-review.json');
...
var expectedAuthorityIds = [
  'ddec-emprender-puerto-rico-2026',
  'ddec-ayudas-incentivos-2026',
  'prdoh-sbia-program-2026'
];

assert.strictEqual(artifact.schemaVersion, 1, 'artifact schemaVersion must be 1');
assert.strictEqual(artifact.artifactVersion, 'aguada-restaurant-support-network-review-v1', 'artifactVersion mismatch');
...
assert.strictEqual(artifact.reviewedAt, '2026-08-20', 'artifact reviewedAt mismatch');
assert.strictEqual(artifact.signalId, 'growth-network-gap', 'artifact signalId mismatch');
assert(signal, 'artifact signalId must exist in the decision-signal registry');
assert.strictEqual(signal.sourceType, 'registered-source', 'ecosystem signal must be registered-source');
assert.strictEqual(signal.sourceId, 'ddec-emprender-puerto-rico-2026', 'ecosystem signal sourceId mismatch');
...
assert.deepStrictEqual(
  ...
  'authority sourceIds must follow the reviewed ecosystem-support stack'
);
...
artifact.authorities.forEach(function(authority) {
  ...
  assert.strictEqual(source.targetTables[0], 'planning-profile-ecosystem', authority.sourceId + ' must target planning-profile-ecosystem');
...
assert(signal.artifactPaths.indexOf('data/profile-reach/aguada-restaurant-support-network-review.json') !== -1, 'signal must link to the reviewed ecosystem artifact');

assert(Array.isArray(artifact.scenarioCoverage), 'scenarioCoverage must be an array');
assert.strictEqual(artifact.scenarioCoverage.length, 2, 'scenarioCoverage must cover medium-regional and large-strategic');
artifact.scenarioCoverage.forEach(function(entry) {
  ...
  assert(
    entry.reachId === 'regional-corridor' || entry.reachId === 'island-wide',
    'unexpected scenarioCoverage reachId for ' + entry.scenarioId
  );
  assert.strictEqual(entry.coverage, 'descriptive-support-network-baseline', 'scenarioCoverage coverage mismatch for ' + entry.scenarioId);
