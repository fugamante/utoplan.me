...
var artifactPath = path.join(__dirname, '..', 'data', 'profile-reach', 'aguada-restaurant-site-screening-review.json');
...
assert.strictEqual(artifact.artifactVersion, 'aguada-restaurant-site-screening-review-v1', 'artifactVersion mismatch');
...
assert.strictEqual(artifact.signalId, 'site-ready-kitchen-gap', 'artifact signalId mismatch');
assert(signal, 'artifact signalId must exist in the decision-signal registry');
assert.strictEqual(signal.sourceType, 'registered-source', 'site-feasibility signal must be registered-source');
...
assert.deepStrictEqual(
  ...
  'authority sourceIds must follow the reviewed site-feasibility stack'
);
...
assert(signal.artifactPaths.indexOf('data/profile-reach/aguada-restaurant-site-screening-review.json') !== -1, 'signal must link to the reviewed site-screening artifact');
...
artifact.scenarioCoverage.forEach(function(entry) {
  ...
  assert.strictEqual(entry.coverage, 'descriptive-site-screening-baseline', 'scenarioCoverage coverage mismatch for ' + entry.scenarioId);
